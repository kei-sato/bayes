 ...
 * @param  {Object} options   optional. see Naivebayes for details.
...
module.exports.fromJson = function (jsonStr, options) {
  ...
  var classifier = new Naivebayes(options || parsed.options);
 ...
 *
 * @param  {String} text
 * @return {{ category: string, confidence: number }} classified category and its confidence
 */
Naivebayes.prototype.categorize = function (text) {
  ...
  .forEach(function (category, i, categoryNames) {

    //start by calculating the overall probability of this category
    //=>  out of all documents we've ever looked at, how many were
    //    mapped to this category
    //with laplase smooting (http://classes.engr.oregonstate.edu/eecs/spring2012/cs534/notes/Naivebayes-6.pdf p18)
    var categoryProbability = (self.docCount[category] + 1) / (self.totalDocuments + categoryNames.length);
    ...
    sumProbability += Math.exp(logProbability);
  });

  var confidence = Math.exp(maxProbability) / sumProbability; // 0 ~ 1

  return { category:chosenCategory, confidence:confidence };
};
